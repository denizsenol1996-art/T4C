...
function render(){
  ...
  if(m&&m.count>0){
    ...
    mktHtml=`<div class="res-sec" >
      ...
      <div id="comp-list-ui"></div>
    </div>`;
  }

  
    // ═══ COMPARABLES UI — vul na render ═══
    setTimeout(() => {
      const compEl = document.getElementById('comp-list-ui')
      const comps = (r.compEngine?.comparables || []).filter(c => c.parsedPrice > 0).slice(0, 8)
      if (!compEl || !comps.length) return
      let h = '<div style="font-size:11px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">Top vergelijkbare auto\'s</div>'
      comps.forEach(c => {
        const opts = c.options ? c.options.split(',').slice(0, 4).map(o => o.trim()).filter(o => o.length > 1) : []
        const badges = [c.transmission, c.fuel, ...opts].filter(Boolean).slice(0, 6)
        h += '<div style="background:var(--bg3,var(--bg2));border:1px solid var(--border);border-radius:8px;padding:8px 10px;margin-bottom:6px">'
        h += '<div style="display:flex;justify-content:space-between;align-items:center">'
        h += '<div style="font-size:12px;font-weight:600;color:var(--text1);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + (c.title || '?') + '</div>'
        h += '<div style="font-size:13px;font-weight:800;color:var(--green);margin-left:8px">' + E(c.parsedPrice) + '</div>'
        h += '</div>'
        h += '<div style="display:flex;gap:6px;align-items:center;margin-top:4px;flex-wrap:wrap">'
        if (c.km) h += '<span style="font-size:10px;color:var(--text3)">' + Math.round(c.km / 1000) + 'k km</span>'
        if (c.year) h += '<span style="font-size:10px;color:var(--text3)">' + c.year + '</span>'
        h += '<span style="font-size:10px;color:var(--text4);border-left:1px solid var(--border);padding-left:6px;margin-left:2px">' + Math.round(c.score) + '%</span>'
        h += '</div>'
        if (badges.length) {
          h += '<div style="display:flex;flex-wrap:wrap;gap:3px;margin-top:5px">'
          badges.forEach(b => { h += '<span style="background:var(--card,var(--bg2));border:1px solid var(--border);border-radius:3px;padding:1px 5px;font-size:9px;color:var(--text3)">' + b + '</span>' })
          h += '</div>'
        }
        h += '</div>'
      })
