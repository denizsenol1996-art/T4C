...
const stmts = {
  ...
  saveTaxatie: {
    run: (d) => run(`INSERT INTO taxaties (kenteken,make,model,model_variant,year,fuel,km,color,body,
      ...
      price_agreement_status,price_agreement_delta_pct,price_source,staat_factor,response_ms,
      shadow_bod,shadow_source)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [d.kenteken,d.make,d.model,d.model_variant,d.year,d.fuel,d.km,d.color,d.body,
       ...
       d.confidence_reasons||null,
       d.user_staat||null,
       d.user_rijdt||null,
       d.taxatie_type||null,
       d.model_platform||null,
       d.expert_verkoop_low==null?null:d.expert_verkoop_low,
       d.expert_verkoop_high==null?null:d.expert_verkoop_high,
       d.expert_bod_low==null?null:d.expert_bod_low,
       d.expert_bod_high==null?null:d.expert_bod_high,
       ...
       d.response_ms==null?null:d.response_ms,
       d.shadow_bod==null?null:d.shadow_bod,
       d.shadow_source||null])
  },
  ...
  upsertListing: {
    run: (hash, mk, ml, yr, title, price, km, trans, source, url, dealer, image_url, options) => {
  ...
  getCrawlQueue: { all: (limit) => queryAll("SELECT q.*, COALESCE(t.cnt, 0) as priority FROM crawl_queue q LEFT JOIN (SELECT LOWER(make) as make, LOWER(model) as model, COUNT(*) as cnt FROM taxaties GROUP BY LOWER(make), LOWER(model)) t ON q.make=t.make AND q.model=t.model ORDER BY COALESCE(t.cnt, 0) DESC, q.last_crawled_at ASC LIMIT ?", [limit]) },
