  ...
  try {
    ...
    const cars = dvCars.map(d => {
      ...
      return {
        ...
        km: d.km || d.tellerstand || 0,
        color: d.basiskleur || d.kleur || '',
        body: d.carrosserie || '',
        power_hp: d.vermogen_pk,
        power_kw: d.vermogen_kw,
        transmission: d.transmissie === 'A' ? 'Automaat' : d.transmissie === 'H' ? 'Handgeschakeld' : '',
        transmission_speeds: d.aantal_versnellingen,
        vraag_prijs: Number(d.prijs) || Number(d.meeneemprijs) || 0,
        nieuwprijs: d.nieuwprijs,
        beschrijving: (d.opmerkingen || '').replace(/<br\s*\/?>/gi, '\n'),
        apk_until: d.apk_tot || '',
        status: 'te_koop',
        ...
        doors: d.aantal_deuren || d.deuren,
        seats: d.aantal_zitplaatsen || d.zitplaatsen,
        btw_type: d.prijs_type === 'in' ? 'BTW-auto' : d.prijs_type === 'ex' ? 'BTW-marge' : d.btw_marge || '',
        engine_cc: d.cilinder_inhoud,
        co2: d.co2_uitstoot,
        energielabel: d.energielabel,
        topsnelheid: d.topsnelheid,
        verbruik: d.verbruik,
        gewicht: d.massa,
        trekgewicht: d.max_trekgewicht,
        nap: d.nap_weblabel === 'j',
        schadevoertuig: d.schadevoertuig === 'j',
        locatie: d.locatie,
        bron: 'dv',
...
router.get("/api/public/voorraad/:id", (req, res) => {
  try {
    ...
    const car = {
      ...
      make: d.merk, model: d.model, model_variant: d.type || '', uitrustingsniveau: d.uitrustingsniveau || '',
      year: d.bouwjaar, modeljaar: d.modeljaar || null,
      fuel: d.brandstof === 'B' ? 'Benzine' : d.brandstof === 'D' ? 'Diesel' : d.brandstof === 'E' ? 'Elektrisch' : d.brandstof === 'L' ? 'LPG' : d.brandstof === 'H' ? 'Hybride' : d.brandstof || '',
      km: d.km || null,
      color: d.kleur || d.basiskleur || '',
      laksoort: d.laksoort || '', bekleding: d.bekleding || '', interieurkleur: d.basisinterieurkleur || '',
      body: d.carrosserie || '',
      power_hp: d.vermogen_pk || null, power_kw: d.vermogen_kw || null,
      transmission: d.transmissie === 'A' ? 'Automaat' : d.transmissie === 'H' ? 'Handgeschakeld' : '',
      versnellingen: d.aantal_versnellingen || null,
      cilinderinhoud: d.cilinder_inhoud || null, cilinders: d.cilinder_aantal || null,
      massa: d.massa || null, trekgewicht: d.max_trekgewicht || null,
      topsnelheid: d.topsnelheid || null, acceleratie: d.acceleratie || null,
      verbruik: d.verbruik || null, co2: d.co2_uitstoot || null,
      emissieklasse: d.emissieklasse || '', energielabel: d.energielabel || '',
      actieradius: d.actieradius_elektrisch || null,
      vraag_prijs: Number(d.prijs) || Number(d.actieprijs) || 0,
      nieuwprijs: d.nieuwprijs || null, bpm: d.bpm || null, rest_bpm: d.rest_bpm || null,
      beschrijving: d.opmerkingen || '', highlights: d.highlights || '', titel: d.titel || '',
      apk_until: d.apk_tot || '', vin: d.vin || '',
      eigenaren: d.aantal_eigenaren || null, sleutels: d.aantal_sleutels || null,
      onderhoudsboekjes: d.onderhoudsboekjes || '', nap: d.nap_weblabel === 'j',
      locatie: d.locatie || '',
      schade: d.schadevoertuig === 'j', schade_status: d.schade_status || '',
      status: 'te_koop',
      cover_photo: fotos.length > 0 ? fotos[0].url : '',
      photos: JSON.stringify(fotos.map(f => f.url)),
      options: opties.length > 0 ? JSON.stringify(opties) : '',
      doors: d.aantal_deuren || null, seats: d.aantal_zitplaatsen || null,
      btw_type: d.btw_marge === 'M' ? 'Marge' : d.btw_marge === 'B' ? 'BTW' : (d.btw_marge || '')
    }
